...
export default function Quiz() {
  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke('generate-quiz', {
      body: {
        ...
        // Keep your original backend key
        mcqCount: questionCount,

        // Extra safe keys in case backend function expects another name
      ...
      <section className="min-h-screen p-8 text-white">
        ...
        <div className="mx-auto max-w-lg rounded-2xl border border-amber-400/30 bg-amber-400/5 p-8 backdrop-blur">
          ...
          <button onClick={handleGenerate} disabled={generating}
            className="w-full rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-3 font-bold transition hover:scale-105 disabled:opacity-50">
  ...
  {q.options && q.options.length > 0 ? (
    q.options.map((opt, oi) => (
      <label
        key={oi}
        className={`flex cursor-pointer items-center gap-3 rounded-xl border px-4 py-3 text-md transition ${
          answers[i] === opt
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <div className="space-y-4 text-left">
            {(results.results || []).map((r, i) => (
              <div key={i} className={`rounded-2xl border p-5 backdrop-blur ${
                r.isCorrect ? 'border-green-400/30 bg-green-400/5' : 'border-red-400/30 bg-red-400/5'
              }`}>
    ...
    <section className="min-h-screen p-8 text-white">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-lg text-slate-400">{quizzes.length} quizzes generated</p>
        </div>
        <button onClick={() => { setStep('generate'); setGenerateForm({ difficulty: 'medium', numQuestions: 5 }); }}
          className="rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 px-6 py-3 font-bold transition hover:scale-105 shadow-[0_0_20px_rgba(255,171,0,.3)]">
          + New Quiz
        </button>
      </div>
